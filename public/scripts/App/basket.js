...
async function addBasketToDom() {
    ...
    if (allBasket) {
        ...
        filteredUserBasket.forEach(function (basket) {
            ...
            newDivElem.className = 'rounded-md sm:rounded-xl bg-zinc-900 overflow-hidden'
            newDivElem.innerHTML = `<div class="rounded-md sm:rounded-xl bg-zinc-900 overflow-hidden">
                        <a href="#"
                           class="flex items-center justify-center w-full h-[120px] sm:h-[160px] md:h-36 rounded-md overflow-hidden">
                            <img class="w-full h-full object-cover" src="${basket[1].img}" alt=""
                                 loading="lazy">
                        </a>
                        <div class="p-2">
                            <h4 class="text-sm font-IRANSans-Medium line-clamp-2 mb-2 sm:mb-4 h-10">
                                چای سیاه یا چای قرمز یکی از محبوب‌‌ترین طعم‌های چای است.طعم‌های چای است.
                            </h4>
                            <div class="flex flex-col-reverse sm:flex-row justify-between items-center gap-y-2">
                                <div class="flex items-center justify-between w-full">
                                    <div class="flex items-center gap-x-2 bg-transparent text-white rounded-md w-28 sm:w-auto p-1">
                                        ...
                                        <button onclick='basketMinusCountAction("${basket[0]}", addBasketToDom, basketPriceHandler)' type="button" class="p-0.5 border border-indigo-500 rounded-full">
                                            <svg class="shrink-0 w-4 h-4 text-indigo-500">
                                                <use href="#plus"></use>
                                            </svg>
                                        </button>
                                    </div>
                                    <span onclick='removeProductFromBasket("${basket[0]}")' class="sm:hidden">
                                        <svg class="w-6 h-6 text-red-600">
                                            <use href="#x-mark"></use>
                                        </svg>
                                    </span>
                                </div>
                                <div class="flex flex-row-reverse sm:flex-col justify-between items-start sm:items-end flex-wrap gap-x-4 w-full">
                                    <div class="product__price flex items-center gap-x-1 font-IRANSans-Bold tracking-wider">
                                        ${basket[1].costPrice.toLocaleString()}
                                    </div>
                                    <div class="offer flex items-center gap-x-1 font-IRANSans-Bold tracking-wider text-gray-400">
                        ...
                        <button onclick='removeProductFromBasket("${basket[0]}")'
                                class="text-gray-100 sm:mt-0.5 md:text-gray-300 items-center justify-center md:hover:text-white transition-colors w-full h-10 md:h-12 bg-red-600 font-IRANSans-Medium rounded-md text-sm md:text-base tracking-tighter hidden sm:flex">
                            حذف محصول
                        </button>
                    </div>`
