        ...
        .then(allComments => {
            ...
            if (allComments) {
                allComments.forEach(comment => {
                    ...
                    if (comment[1].status) {
                        newCommentRow.innerHTML = `<td><span onclick='removeComment("${comment[0]}")' class="text-red-700 cursor-pointer flex items-center justify-center"><svg class="w-6 h-6"><use href="#x-mark"></use></svg></span></td><td class="hidden lg:table-cell">${comment[1].email}</td><td><button onclick="showCommentText('${comment[1].text}')" class="bg-indigo-400 rounded py-1 md:py-2 px-2 md:px-4" type="button">نمایش</button></td><td class='w-[152px]'><span onclick='commentCheck("${comment[0]}")' class="comments-status__icon text-indigo-600 cursor-pointer block comments-status--checked">تایید شد</span></td>`
                        commentsFragment.append(newCommentRow)
                    } else {
                        newCommentRow.innerHTML = `<td><span onclick='removeComment("${comment[0]}")' class="text-red-700 cursor-pointer flex items-center justify-center"><svg class="w-6 h-6"><use href="#x-mark"></use></svg></span></td><td class="hidden lg:table-cell">${comment[1].email}</td><td><button onclick="showCommentText('${comment[1].text}')" class="bg-indigo-400 rounded py-1 md:py-2 px-2 md:px-4" type="button">نمایش</button></td><td><span onclick='commentCheck("${comment[0]}")' class="comments-status__icon text-red-700 cursor-pointer block comments-status--checked">تایید نشده</span></td>`
                        commentsFragment.append(newCommentRow)
                    }
                })
